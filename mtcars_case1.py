...
#Filter Rows
#cars with cyl=8
mtcars.loc[mtcars['cyl']==8]
#cars with mpg <= 27
mtcars.loc[mtcars['mpg']<=27]
list(mtcars.index)
#rows match auto tx
...
mtcars.mpg.apply({'mpg': lambda x: x>23,'wt': lambda x: x<2})
...
mtcars2=mtcars2.disp.apply({'disp': lambda x: x/61 })
# multiple mpg * 1.5 and save as original column
mtcars2=mtcars.mpg.apply({'mpg': lambda x: x*1.5})
